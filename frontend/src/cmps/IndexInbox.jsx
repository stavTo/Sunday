...
export function IndexInbox() {
    ...
    return (
        <section className="inbox p-1em">
            {/* <h1 className="fs18 row gap-half"> */}
            <h1 className="fs18 flex row gap-half text-cursor align-center">
                <div className={`${toggle ? 'expand-arrow-container pointer' : 'close-arrow-container pointer'}`} onClick={() => setToggle(!toggle)}>
                    {ICON_EXPAND_ARROW}
                </div>Inbox <span className="inbox-indication flex align-center justify-center wght-400">0</span>
            </h1>
            {/* <div className={`${toggle ? 'expand-arrow-container pointer' : 'close-arrow-container pointer'}`} onClick={() => setToggle(!toggle)}>
                    {ICON_EXPAND_ARROW}
                </div>Inbox
            ...
            {toggle &&
                <div className="inbox-container flex column">
                    <img className="empty-inbox flex justify-center" src={emptyInbox} />
                    <div className="flex column fs-15 align-center space-between">
                        <span>High Five!</span>
